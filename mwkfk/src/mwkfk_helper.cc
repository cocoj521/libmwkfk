...
LUtil::Logger::LOG_LEVEL MWkfkHelper::kLogLevel = LUtil::Logger::LL_INFO;
bool MWkfkHelper::kInitLog = false;
static std::mutex kInitLogMutex;

void MWkfkHelper::InitLog(const std::string& log_level, const std::string& log_path) 
{
	...
	std::lock_guard<std::mutex> lock(kInitLogMutex);
...
void MWkfkHelper::InitLog(LUtil::Logger::LOG_LEVEL log_level, const std::string& log_path) 
{
    std::lock_guard<std::mutex> lock(kInitLogMutex);
...
bool MWkfkHelper::GetMWkfkBrokerList(const MWkfkConfigLoader& config_loader, std::string* broker_list) 
{
    bool rt = false;
...
bool MWkfkHelper::SetRdKafkaConfig(rd_kafka_conf_t* rd_kafka_conf, const char* item, const char* value) 
{
	bool rt = false;
		...
		&& 0 != item[0]) 
	{
		...
		{
			ERROR(__FUNCTION__ 
				<< " | Failed to rd_kafka_conf_set | item: " << item
				<< " | value: " << value
				<< " | error msg: " << err_str);
		} 
		else 
...
bool MWkfkHelper::SetRdKafkaTopicConfig(rd_kafka_topic_conf_t* rd_kafka_topic_conf, const char* item, const char* value) 
{
	bool rt = false;
...
std::string MWkfkHelper::FormatTopicPartitionList(const rd_kafka_topic_partition_list_t* partitions) 
{
    std::string rt("");
...
void MWkfkHelper::RdKafkaLogger(const rd_kafka_t *rk, int level, const char *fac, const char *buf) 
{
    time_t timep;
    time (&timep);
...
void MWkfkHelper::SetClientId(rd_kafka_conf_t* rd_kafka_conf, bool isAppendThreadId) 
{
    if (isAppendThreadId) 
	{
        char clientId[100];
...
int64_t MWkfkHelper::GetCurrentTimeMs() 
{
    struct timeval now_time; 
    gettimeofday(&now_time, NULL); 
    return ((int64_t)now_time.tv_sec) * 1000 + (int64_t)now_time.tv_usec/1000;
}
